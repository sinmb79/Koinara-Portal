...
function mergeWallets(wallets) {
  ...
  wallets.forEach((wallet, index) => {
    const key = wallet.rdns || wallet.id || wallet.name || `wallet-${index}`
    ...
    const existingScore = (existing.rdns ? 2 : 0) + (existing.name ? 1 : 0)
    const nextScore = (wallet.rdns ? 2 : 0) + (wallet.name ? 1 : 0)
